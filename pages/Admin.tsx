...
import { Users, ShieldAlert, Search, ArrowRight, UserPlus, Edit, Trash2, X, Mail, Phone, User as UserIcon, MapPin, Wallet, Check, Loader2, AlertTriangle } from 'lucide-react';
...
const AdminPage: React.FC<{currentUser: UserProfile, onNotify: (m: string, type?: 'success' | 'error') => void}> = ({ currentUser, onNotify }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      setListError("Users list can't be loaded. Check Firebase Rules.");
    });
    return () => unsub();
  }, []);
  ...
  const handleGrantAdmin = async () => {
    ...
    try {
      ...
      onNotify("Error: " + e.message, "error");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setProcessing(true);
    try {
      // FULL ACCESS: Admin can update EVERYTHING
      ...
      onNotify("Database updated successfully!");
      setEditingUser(null);
    } catch (e: any) {
      onNotify("Critical Update Error: " + e.message, "error");
    } finally {
      setProcessing(false);
    }
  };
        ...
        <div className="xl:col-span-1 space-y-6">
          ...
          <div className="bg-blue-900 p-8 rounded-[3rem] text-white shadow-3xl relative overflow-hidden">
            ...
            <div className="mt-4 flex items-center gap-2 text-[10px] font-black uppercase bg-white/20 p-2 rounded-xl">
              <UserPlus size={14} /> Realtime Database
            </div>
          </div>
        </div>
          ...
          <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              <h3 className="font-black text-blue-950 uppercase text-xs flex items-center gap-2">
                <Users size={16} className="text-blue-600" /> Merchant Database
              </h3>
              {listError && <p className="text-[10px] font-black text-red-500 uppercase mt-1 flex items-center gap-1"><AlertTriangle size={10}/> {listError}</p>}
            </div>
                ...
                {filtered.length > 0 ? filtered.map(u => (
                  <tr key={u.uid} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-10 py-6 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[8px] font-black ${u.isAdmin ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-400'}`}>
                        {u.isAdmin ? 'YES' : 'NO'}
                      </span>
                    </td>
                    <td className="px-10 py-6 text-center">
                  ...
                  <tr>
                    <td colSpan={4} className="p-20 text-center">
                      <p className="text-xs font-black text-slate-300 uppercase tracking-widest">No users found or error loading list</p>
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-[600] bg-blue-950/90 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-2xl rounded-[4rem] p-12 shadow-4xl relative overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <h2 className="text-3xl font-black text-blue-950 uppercase mb-8">Admin Control Panel</h2>
            ...
            <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-orange-500 uppercase flex items-center gap-2 font-black"><Mail size={12}/> Account Email (Sensitive)</label>
                <input required type="email" className="w-full p-4 bg-orange-50 border border-orange-100 rounded-2xl font-black text-sm outline-none focus:border-orange-500"
              ...
              <div className="space-y-2">
                ...
                <input type="number" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-sm outline-none focus:border-blue-500"
                  value={editingUser.walletBalance} onChange={e => setEditingUser({...editingUser, walletBalance: Number(e.target.value)})} />
              </div>

              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><MapPin size={12}/> Store Address</label>
                <textarea rows={2} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm resize-none outline-none"
                  value={editingUser.address || ''} onChange={e => setEditingUser({...editingUser, address: e.target.value})} />
              </div>

              <div className="md:col-span-2 flex items-center gap-3 bg-blue-50 p-4 rounded-2xl">
                <input type="checkbox" id="admin_check" checked={editingUser.isAdmin} onChange={e => setEditingUser({...editingUser, isAdmin: e.target.checked})} className="w-5 h-5 accent-blue-900" />
                <label htmlFor="admin_check" className="text-xs font-black text-blue-950 uppercase">Admin Access Enabled</label>
              </div>

              <div className="md:col-span-2 pt-6">
                <button type="submit" disabled={processing} className="w-full py-6 bg-blue-900 text-white rounded-[2rem] font-black uppercase text-xs shadow-2xl flex items-center justify-center gap-3 disabled:opacity-50">
                  {processing ? <Loader2 className="animate-spin" size={18} /> : <><Check size={18} /> Save All User Data</>}
                </button>
              </div>
            </form>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[700] bg-red-950/90 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-12 text-center animate-in zoom-in-95">
            ...
            <h2 className="text-2xl font-black text-slate-900 uppercase mb-4">Delete User?</h2>
            <p className="text-slate-400 font-bold mb-10 text-xs">This account will be permanently erased from Firestore.</p>
            <div className="flex gap-4">
              <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black uppercase text-[10px]">Cancel</button>
              <button onClick={() => handleDeleteUser(showDeleteConfirm)} disabled={processing} className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-[10px]">
                {processing ? "Erasing..." : "Confirm Delete"}
              </button>
            </div>
          </div>
        </div>
